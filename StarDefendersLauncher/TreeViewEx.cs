...
namespace StarDefendersLauncher
{
    public static class TreeViewEx
    {
        ...
        public static void AddObjectAsJson<T>(this TreeView tv, T obj, string nodeName)
        {
            tv.BeginUpdate();
            try
            {
                var s = new Stack<IndexContainer>();
                s.Push(new IndexContainer());
                FillTreeView(tv.Nodes.Add(nodeName), JsonConvert.DeserializeObject<JToken>(JsonConvert.SerializeObject(obj)), s);
                s.Pop();
            }
            finally
            {
